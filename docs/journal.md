...
- 2026-04-14 — Landed Batch 5 from [docs/review/REVIEW.md](review/REVIEW.md) on `fix/batch-5-a11y-polish`: accessibility polish pass. 5 atomic commits.
  ...
  - Weak spots: the custom B/S rule `<input>` inside the rule popover is a text input, not a button — so the rule popover's `role="menu"` is technically mixed-content. Considered changing to `role="dialog"`, but that's a bigger semantic pivot and the menu announcement is still useful for the preset options. Tracked as a future polish item.
