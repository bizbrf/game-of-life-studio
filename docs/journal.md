...
- 2026-04-14 — Landed Batch 1 from [docs/review/REVIEW.md](docs/review/REVIEW.md) on `fix/batch-1-known-wart-leak`: the known-wart leak + doc reality. 7 atomic commits.
  - Broke the `state.js ↔ rules.js` circular import: `state.rules` is now initialised to `null` at module-load time and populated by `applyRule(state.rule)` during `app.js:initialize()`.
  - Hoisted `showToast` and direct DOM writes out of the middle layer. `applyRule`, `undo`, `redo`, `randomFill` now return `{ success, message, ... }` result objects; `app.js` orchestrator (`applyRuleAndToast` helper and refactored button handlers) surfaces toasts and calls `updateUI()`. `ui.js:updateUI` picks up the `els.exportNote` projection that used to live in `rules.js`.
  - Hoisted `updateUI` out of `tools.js:setTool` / `selectPattern`; callers in `ui.js` (tool grid, pattern card) and `app.js` (keyboard) now drive the UI sync. Also fixed `tools.dedupeCells` to use `keyFromXY` (ARCHITECTURE invariant 5) instead of a hand-assembled `${x},${y}` template.
  - Moved `handleKeydown` from `input.js` to `app.js`, which was the single biggest structural source of upward imports. `input.js` now imports zero symbols from `ui.js` / `audio.js` / `history.js`; its imports span 6 modules: `constants`, `state`, `utils`, `tools`, `sim`, `render` (the review's baseline module-map count was 3, actual was 9 modules pre-move). `beginInteraction` inlines the modal-open check (`state.modals.size > 0`).
  - Docs refreshed to match reality: [docs/agents/module-map.md](agents/module-map.md) now lists accurate imports for every module (the previous table had `input.js` at 3 imports vs. the actual 9; `sim.js` row missed `history` / `audio` / `render`; `rules.js` row still carried a `ui*` footnote); public-surface entries gained the new return shapes; `ARCHITECTURE.md`'s Known Warts section retired the `rules.js → showToast` entry and added the two that remain for Batch 4 (`sim.randomFill → visibleWorldBounds`, middle-layer DOM access via `themes.js` / `render.js`). [docs/adr/0003-module-split-boundaries.md](adr/0003-module-split-boundaries.md) and [game-of-life-v2/scripts/AGENTS.md](../game-of-life-v2/scripts/AGENTS.md) amended to remove the "one known exception" language.
  - Verification: headless Chromium via [`%TEMP%/gol-ui-check/check-batch1.js`](../../../AppData/Local/Temp/gol-ui-check/check-batch1.js). Boot compiles `state.rules` via `applyRule`; invalid rule input toasts + leaves `state.rule` unchanged; Ctrl+Z on empty undo toasts "Nothing to undo"; keyboard `4` switches to line tool; `F` toasts random-fill message; `Tab` cycles `state.patternIndex` (async dynamic-import path still works). `input.js` imports grep'd over the wire: zero matches for `./ui.js`, `./audio.js`, `./history.js`. Zero console errors.
  - Weak spots: `io.js` still imports `applyRule` and uses its return value shape implicitly; that's correct because `io.js` is a caller rather than middle-layer-from-middle-layer, but worth re-reading when Batch 4 touches `themes.js`. `ui.js` still re-exports `hexToRgb` purely as a convenience for `app.js` — not a violation, just a minor wart.
