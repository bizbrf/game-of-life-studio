...
- 2026-04-14 — Landed Batch 1 from [docs/review/REVIEW.md](docs/review/REVIEW.md) on `fix/batch-1-known-wart-leak`: the known-wart leak + doc reality. 7 atomic commits.
  ...
  - Moved `handleKeydown` from `input.js` to `app.js`, which was the single biggest structural source of upward imports. `input.js` now imports zero symbols from `ui.js` / `audio.js` / `history.js`; its 6 imports are `constants`, `state`, `utils`, `tools`, `sim`, `render`. `beginInteraction` inlines the modal-open check (`state.modals.size > 0`).
